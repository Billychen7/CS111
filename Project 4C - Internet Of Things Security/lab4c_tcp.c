...
void turnOff(){
    ...
    ifError = write(1, buf, strlen(buf));
    if (ifError < 0){
        fprintf(stderr, "Error: Failure writing buffer to stdout: %s\n", strerror(errno));
        exit(2);
    }
    if (logFlag == 1){
        ifError = write(logFile, buf, strlen(buf));
        if (ifError < 0){
...
void checkCommands(const char* commandGiven){
	...
	{	
		...
		buf[lastIndex] = '\n';
        int ifError = write(logFile, buf, strlen(commandGiven) + 1);
		if (ifError < 0){
            fprintf(stderr, "Error: Failure writing buffer to log: %s\n", strerror(errno));
            exit(2);
        }	
...
int main(int argc, char *argv[])
{
    ...
    int ifError = connect(client, (struct sockaddr*)&address, sizeof(address));
    if (ifError < 0){
    ...
    ifError = write(1, buf, strlen(buf));
    if (ifError < 0){
        fprintf(stderr, "Error: Failure writing to stdout: %s\n", strerror(errno));
        exit(2);
    }
    if (logFlag == 1)
    {
        ifError = write(logFile, buf, strlen(buf));
        if (ifError < 0){
            fprintf(stderr, "Error: Failure to write to log file: %s \n", strerror(errno));
            exit(2);
        }
    }
    ...
    while(1){
        ...
        if (nextTime.tv_sec <= currTime.tv_sec && continueCheck == 1)
        {
            ...
            ifError = write(1, buffer, strlen(buffer));
            if (ifError < 0){
                fprintf(stderr, "Error: Failure writing buffer to stdout: %s\n", strerror(errno));
                exit(2);
            }
            if (logFlag == 1){
                ifError = write(logFile, buffer, strlen(buffer));
                if (ifError < 0){
                    fprintf(stderr, "Error: Failure writing buffer to log: %s\n", strerror(errno));
                    exit(2);
                }
